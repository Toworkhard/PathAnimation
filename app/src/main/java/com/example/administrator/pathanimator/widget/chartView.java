...
//    private int xytextcolor = 0xff7e7e7e;
    private int xytextcolor = Color.parseColor("#aaaaaa");
    //xy坐标轴文字大小
    private int xytextsize = spToPx(10);
    //折线图中折线的颜色
//    private int linecolor = 0xff02bbb7;
    private int linecolor = Color.parseColor("#5071e4");
    //x轴各个坐标点水平间距
    ...
    //y轴坐标对应的数据
    private List<String> yValuetext = new ArrayList<>();
    //y轴坐标对应的数据
    ...
    private void initPaint() {
        ...
        DashPaint =  new Paint();
        DashPaint.setColor (xylinecolor) ;
        //设置画直线格式
...
//            //X轴文本最大高度
            ...
            yOri = (int) (height - dp5*2 - textXHeight*2 - dp3 - xylinewidth);//dp3是x轴文本距离底边，dp2是x轴文本距离x轴的距离
//            xInit = interval + xOri;
            xInit = xOri + dpToPx(25);
...
//        super.onDraw(canvas);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            float y = yOri - yOri * (1 - 0.15f) * (Float.parseFloat(value.get(xValue.get(i)))*100)/ yValue.get(yValue.size() - 1);
            //绘制选中的点
            if (i == selectIndex - 1) {
                linePaint.setStyle(Paint.Style.FILL);
                //linePaint.setColor(0xffd0f3f2);
                linePaint.setColor(Color.parseColor("#cc5276fc"));
    ...
    private void drawFloatTextBox(Canvas canvas, float x, float y, String text) {
        ...
        linePaint.setStyle(Paint.Style.FILL);
        linePaint.setColor(Color.parseColor("#5071e4"));
...
//        //绘制折线
        ...
        float y = yOri - yOri * (1 - 0.15f) * (Float.parseFloat(value.get(xValue.get(0)))*100) / yValue.get(yValue.size() - 1);
        path.moveTo(x, y);
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < yValue.size(); i++) {
            ...
            xyTextPaint.setColor(xytextcolor);
            //绘制Y轴文本
            String text = yValuetext.get(i)+"%";
            Rect rect = getTextBounds(text, xyTextPaint);
            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dpToPx(2) - rect.width() + 20, yOri - yLength * i + rect.height() / 2, xyTextPaint);
            ...
            if(i !=0) {
                ...
                canvas.drawLine(text.length(), yOri - yLength * i + rect.height() / 2, xLength + length, yOri - yLength * i + rect.height() / 2, DashPaint);
            }
        }
        //绘制X轴坐标
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                ...
                Rect rect = getTextBounds(text, xyTextPaint);
                String year = YearValue.get(i);
                Rect rect2 = getTextBounds(year, xyTextPaint);
//                if (i == selectIndex - 1) {
...
//                canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
                ...
                canvas.drawText(year, 0, year.length(), x - rect2.width() / 2, yOri + xylinewidth + dpToPx(10) + rect.height() + rect2.height(), xyTextPaint);
            }
        }
    }
...
//        this.getParent().requestDisallowInterceptTouchEvent(true);//当该view获得点击事件，就请求父控件不拦截事件
    ...
    public void setValue(Map<String, String> value, List<String> xValue, List<String> yValue,List<String> YearValue) {
        this.value = value;
        this.xValue = xValue;
        this.yValuetext = yValue;
        this.YearValue =YearValue;
        this.yValue.clear();
        for(int i = 0; i<yValue.size();i++){
            String y = yValue.get(i);
            this.yValue.add((int)(Float.parseFloat(y)*100));
        }
        invalidate();
    }
